    ...
    def forward(self, entry, testing=False):
        ...
        for i in range(k - 1, 0, -1):
            ...
            if diff > 1:
                frame_ranges = torch.cat(
                    (frame_ranges[: i],
                     torch.tensor([frame_ranges[i] for j in range(diff - 1)]).to(device=im_idx.device),
                     frame_ranges[i:]))
        if im_idx[0] > 0:
            frame_ranges = torch.cat(
    ...
    def forward_single_entry(self, context_fraction, entry):
        ...
        if self.mode == "predcls":
            ...
            pred["pred_scores"] = entry["pred_scores"][
                                  torch.min(pair_idx[frame_ranges[end]: frame_ranges[end + 1]]): torch.max(
                                      pair_idx[frame_ranges[end]: frame_ranges[end + 1]]) + 1].repeat(
                num_frames - end - 1)
        ...
        pred["im_idx"] = torch.tensor(
            [i for i in range(num_frames - end - 1) for j in range(frame_ranges[end + 1] - frame_ranges[end])],
            dtype=torch.int32).to(device=frame_ranges.device)
        mx = torch.max(pair_idx[frame_ranges[end]: frame_ranges[end + 1]]) - torch.min(
            pair_idx[frame_ranges[end]: frame_ranges[end + 1]]) + 1
        pred["pair_idx"] = (pair_idx[frame_ranges[end]: frame_ranges[end + 1]] - torch.min(
            pair_idx[frame_ranges[end]: frame_ranges[end + 1]])).repeat(
