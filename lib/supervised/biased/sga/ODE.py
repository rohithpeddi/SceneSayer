...
class ODE(nn.Module):
	...
	def forward_single_entry(self, context_fraction, entry):
		...
		add["future_end"] = n
		indices = torch.reshape((im_idx[: -1] != im_idx[1:]).nonzero(), (-1,)) + 1
		rng = torch.cat((torch.Tensor([0]).to(device=indices.device), indices,
		                 torch.Tensor([tot]).to(device=indices.device))).long()
		add["future_frame_idx"] = [i for i in range(end + 1, n) for j in range(rng[end + 1] - rng[end])]
		ret = odeint(self.diff_func, entry["global_output"][rng[end]: rng[end + 1]], times[end:],
		...
		add["spatial_distribution"] = torch.flatten(torch.sigmoid(self.dsgdetr.s_rel_compress(ret)), start_dim=0,
		                                            end_dim=1)
		add["contact_distribution"] = torch.flatten(torch.sigmoid(self.dsgdetr.c_rel_compress(ret)), start_dim=0,
		                                            end_dim=1)
		if self.mode == "predcls":
			add["scores"] = entry["scores"][torch.min(pair_idx[rng[end]: rng[end + 1]]): torch.max(
				pair_idx[rng[end]: rng[end + 1]]) + 1].repeat(n - end - 1)
			add["labels"] = entry["labels"][torch.min(pair_idx[rng[end]: rng[end + 1]]): torch.max(
				pair_idx[rng[end]: rng[end + 1]]) + 1].repeat(n - end - 1)
		else:
			add["pred_scores"] = entry["pred_scores"][torch.min(pair_idx[rng[end]: rng[end + 1]]): torch.max(
				pair_idx[rng[end]: rng[end + 1]]) + 1].repeat(n - end - 1)
			add["pred_labels"] = entry["pred_labels"][torch.min(pair_idx[rng[end]: rng[end + 1]]): torch.max(
				pair_idx[rng[end]: rng[end + 1]]) + 1].repeat(n - end - 1)
		entry["output"][0] = add
		return entry
