...
class STTran(nn.Module):
    ...
    def forward(self, entry):
        ...
        in_mask = torch.tril(torch.ones(sequence_features.shape[1],sequence_features.shape[1]),diagonal = 0)
        ...
        for i in range(seq_len):
            mask = in_mask[i].unsqueeze(0)
            mask = mask.T.unsqueeze(0)
            mask_input = sequence_features*mask
            rel = self.global_transformer(mask_input,src_key_padding_mask = masks.cuda())
            out[:,i,:] = rel[:,i,:]
        rel_ = out
        rel_ = rel_.cuda()
