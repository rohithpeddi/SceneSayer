...
class STTran(nn.Module):
    ...
    def forward(self, entry, testing = False):
        ...
        for frame in gt_annotation:
            while im_idx[ctr] == prev:
                subject_boxes[ctr] = torch.tensor(frame[pair_idx[ctr, 0] - prev]['person_bbox'], dtype = torch.float32)
                object_boxes[ctr] = torch.tensor(frame[pair_idx[ctr, 1] - prev]['bbox'], dtype = torch.float32)
                ctr += 1
            prev = ctr"""
...
class ODE(nn.Module):
    ...
    def forward(self, entry, testing = False):
        ...
        if not testing or self.mode == "predcls":
            labels = entry["labels"]
            labels_obj = labels[obj]
        else:
        ...
        for i in range(1, self.max_window + 1):
            ...
            if testing:
                ...
                entry["boxes_test_%d" %i] = entry["boxes"][torch.unique(pair_idx[mask_gt], sorted=False, return_inverse=False)]
