...
from train_base import fetch_train_basic_config, prepare_optimizer, save_model, get_sequence_no_tracking, fetch_loss_functions
...
def process_train_video(conf, entry, optimizer, model, epoch, num, tr, gpu_device, dataloader_train):
    ...
    while start + context + 1 <= total_frames:
        ...
        objects_not_in_last_context_frame = appearing_objects.union(disappearing_objects)
        objects_not_in_last_context_frame = list(objects_not_in_last_context_frame)
        ...
        for obj in objects_not_in_last_context_frame:
