...
from lib.supervised.biased.sga.modules.obj_processor_dsgdetr import ObjProcessorDsgDetr
from lib.supervised.biased.sga.modules.obj_processor_sttran import ObjProcessorSTTran
...
def fetch_sequences_after_tracking(conf, entry, gt_annotation, matcher, frame_size):
    if "sttran" in conf.method_name:
        get_sequence_no_tracking(entry, conf)
    elif "dsgdetr" in conf.method_name:
...
def load_object_processor_sttran(conf, dataset, gpu_device):
    model = ObjProcessorSTTran(mode=conf.mode, obj_classes=dataset.object_classes).to(device=gpu_device)
    model = load_model_from_checkpoint(model, conf, gpu_device)
    return model


def load_object_processor_dsgdetr(conf, dataset, gpu_device):
    model = ObjProcessorDsgDetr(mode=conf.mode, obj_classes=dataset.object_classes).to(device=gpu_device)
    model = load_model_from_checkpoint(model, conf, gpu_device)
    return model
...
def load_common_config(conf, ag_train_data, gpu_device):
    ...
    elif conf.method_name == "obj_processor_sttran":
        model = load_object_processor_sttran(conf, ag_train_data, gpu_device)
    elif conf.method_name == "obj_processor_dsgdetr":
        model = load_object_processor_dsgdetr(conf, ag_train_data, gpu_device)
    else:
        raise ValueError(f"Method name {conf.method_name} not recognized")
