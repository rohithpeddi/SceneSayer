...
class GraphDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # print(temp.keys())
        return torch.from_numpy(graph), torch.from_numpy(final_graph), torch.from_numpy(temp['graph']), temp['video_id'], torch.from_numpy(temp['frames'])
...
class GraphModel(nn.Module):
    ...
    def forward(self, x):
        ...
        past_graphs = torch.flatten(x[:, :self.past_len], start_dim=2, end_dim=3) # [Batch size, 6, graph height*graph width]
        gt_graphs = torch.flatten(x[:, self.past_len:], start_dim=2, end_dim=3) # [Batch size, 6, graph height*graph width]
