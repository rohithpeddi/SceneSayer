	...
	with torch.no_grad():
		for entry in tqdm(dataloader_test, position=0, leave=True):
			...
			for i in range(1, w + 1):
				...
				if conf.modified_gt:
					with_constraint_evaluator.evaluate_scene_graph(pred["gt_annotation_" + str(i)][i:], pred_anticipated)
					no_constraint_evaluator.evaluate_scene_graph(pred["gt_annotation_" + str(i)][i:], pred_anticipated)
				else:
					with_constraint_evaluator.evaluate_scene_graph(gt_annotation[i:], pred_anticipated)
					no_constraint_evaluator.evaluate_scene_graph(gt_annotation[i:], pred_anticipated)
