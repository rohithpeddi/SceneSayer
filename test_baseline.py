...
def generate_context_qualitative_results(model, entry, gt_annotation, conf, context_fraction, percentage_evaluators,
    ...
    for frame_idx, pred_tuple_array in with_constraint_predictions_map.items():
        # Loop through each prediction tuple in the frame
        pred_tuple_set = set()
        for idx in range(len(pred_tuple_array)):
            pred_tuple_set.add(tuple(pred_tuple_array[idx]))

        graph = nx.MultiGraph()

        for pred_tuple in pred_tuple_set:
            subject_class = ag_test_data.object_classes[pred_tuple[0]]
            object_class = ag_test_data.object_classes[pred_tuple[1]]
            predicate_class = ag_test_data.relationships[pred_tuple[2]]
