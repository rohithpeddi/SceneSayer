...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph(self, gt, pred,end,future_end,future_frame_idx,count):
        ...
        pred["output"][count]['attention_distribution'] = nn.functional.softmax(pred["output"][count]['attention_distribution'], dim=1)
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            rels_i = np.concatenate((pred['pair_idx'][end:future_end][future_frame_idx == f_idx].cpu().clone().numpy(),             #attention
                                     pred['pair_idx'][end:future_end][future_frame_idx == f_idx].cpu().clone().numpy()[:,::-1],     #spatial
                                     pred['pair_idx'][end:future_end][future_frame_idx == f_idx].cpu().clone().numpy()), axis=0)    #contacting

            pred_scores_1 = np.concatenate((pred["output"][count]['attention_distribution'][future_frame_idx == f_idx].cpu().numpy(),
                                            np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['spatial_distribution'].shape[1]]),
                                            np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['contacting_distribution'].shape[1]])), axis=1)
            pred_scores_2 = np.concatenate((np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['attention_distribution'].shape[1]]),
                                            pred["output"][count]['spatial_distribution'][future_frame_idx == f_idx].cpu().numpy(),
                                            np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['contacting_distribution'].shape[1]])), axis=1)
            pred_scores_3 = np.concatenate((np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['attention_distribution'].shape[1]]),
                                            np.zeros([pred['pair_idx'][end:future_end][future_frame_idx == f_idx].shape[0], pred["output"][count]['spatial_distribution'].shape[1]]),
                                            pred["output"][count]['contacting_distribution'][future_frame_idx == f_idx].cpu().numpy()), axis=1)
            ...
            if self.mode == 'predcls':
                pred_entry = {
                    'pred_boxes': pred['boxes'][(pred["boxes"][:,0]>=boxes_start) & (pred["boxes"][:,0]<=boxes_end)][:,1:].cpu().clone().numpy(),
