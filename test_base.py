...
def modify_pred_dict_disappearance(pred_dict, gt_future):
    ...
    device = pred_dict['labels'].device
    mod_attention_distribution = []
    mod_spatial_distribution = []
    mod_contacting_distribution = []
    mod_global_output = []
    mod_pair_idx = []
    mod_im_idx = []
    mod_labels = []
    ...
    num_clf_objects = (pred_dict['labels'].unique()).shape[0]

    num_ff = len(gt_future)
    for ff_num in range(num_ff):
        # 1. Fetch ground truth object labels of the frame
        gt_ff_info = gt_future[ff_num]
        gt_frame_labels = []
        for num_obj in range(1, len(gt_ff_info)):
            gt_frame_labels.append(gt_ff_info[num_obj]['class'])

        # 2. Use them to modify the pred_dict
        pred_frame_labels = list(
            pred_dict['labels'][ff_num * num_clf_objects: (ff_num + 1) * num_clf_objects].cpu().numpy())

        # 3. Common objects in both gt and pred
        common_objects = list(set(gt_frame_labels).intersection(set(pred_frame_labels)))

        # 4. Construct modified pred_dict using common objects
        mod_frame_labels = [torch.tensor(1, device=device)]
        mod_frame_pred_labels = [torch.tensor(1, device=device)]
        mod_frame_scores = [torch.tensor(1, device=device)]
        mod_frame_pred_scores = [torch.tensor(1, device=device)]
        mod_frame_boxes = [torch.tensor([0.5 for _ in range(5)], device=device)]

        num_prev_boxes = len(mod_labels)
        for c_obj_id, obj in enumerate(common_objects):
            obj_idx = pred_frame_labels.index(obj)
            global_box_idx = ff_num * num_clf_objects + obj_idx
            global_obj_idx = global_box_idx - ff_num - 1

            mod_im_idx.append(torch.tensor(ff_num, device=device))
            mod_pair_idx.append(torch.tensor([num_prev_boxes, num_prev_boxes + 1 + c_obj_id], device=device))
            mod_attention_distribution.append(pred_dict['attention_distribution'][global_obj_idx])
            mod_spatial_distribution.append(pred_dict['spatial_distribution'][global_obj_idx])
            mod_contacting_distribution.append(pred_dict['contacting_distribution'][global_obj_idx])
            mod_global_output.append(pred_dict['global_output'][global_obj_idx])

            mod_frame_labels.append(pred_dict['labels'][global_box_idx])
            mod_frame_pred_labels.append(pred_dict['pred_labels'][global_box_idx])
            mod_frame_scores.append(pred_dict['scores'][global_box_idx])
            mod_frame_pred_scores.append(pred_dict['pred_scores'][global_box_idx])
            mod_frame_boxes.append(pred_dict['boxes'][global_box_idx])

        mod_labels.extend(mod_frame_labels)
        mod_pred_labels.extend(mod_frame_pred_labels)
        mod_scores.extend(mod_frame_scores)
        mod_pred_scores.extend(mod_frame_pred_scores)
        mod_boxes.extend(mod_frame_boxes)

    def update_dict(key, value):
        if value is not None and len(value) > 0:
            mod_pred_dict[key] = torch.stack(value)

    update_dict('attention_distribution', mod_attention_distribution)
    update_dict('spatial_distribution', mod_spatial_distribution)
    update_dict('contacting_distribution', mod_contacting_distribution)
    update_dict('global_output', mod_global_output)
    update_dict('pair_idx', mod_pair_idx)
    update_dict('im_idx', mod_im_idx)
    update_dict('labels', mod_labels)
    update_dict('pred_labels', mod_pred_labels)
    update_dict('scores', mod_scores)
    update_dict('pred_scores', mod_pred_scores)
    update_dict('boxes', mod_boxes)

    return mod_pred_dict



def modify_pred_dict_disappearance_diffeq(pred_dict, gt_future):
    mod_pred_dict = {
        'attention_distribution': [],
        'spatial_distribution': [],
        'contacting_distribution': [],
