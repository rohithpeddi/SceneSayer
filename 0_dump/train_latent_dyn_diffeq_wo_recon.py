...
def train_model(conf, model, matcher, optimizer, dataloader_train, model_ratio, tr, epoch):
    ...
    for entry in tqdm(dataloader_train, position=0, leave=True):
        if const.GT_ANNOTATION not in entry.keys():
            continue
        ...
        # losses["anticipated_latent_loss"] = torch.Tensor([0]).to(torch.float32).to(device=torch.device("cuda:0"))
        losses["anticipated_subject_boxes_loss"] = torch.Tensor([0]).to(torch.float32).to(device=torch.device("cuda:0"))
        losses["anticipated_spatial_relation_loss"] = torch.Tensor([0]).to(torch.float32).to(device=torch.device("cuda:0"))
        losses["anticipated_contact_relation_loss"] = torch.Tensor([0]).to(torch.float32).to(device=torch.device("cuda:0"))
        losses["anticipated_attention_relation_loss"] = torch.Tensor([0]).to(torch.float32).to(device=torch.device("cuda:0"))
...
def main():
    ...
    checkpoint_name = f"{model_name}_wo_recon_{conf.mode}_future_{conf.max_window}"
