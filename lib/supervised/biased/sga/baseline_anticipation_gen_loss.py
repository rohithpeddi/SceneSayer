...
class BaselineWithAnticipationGenLoss(BaseTransformer):
    ...
    def forward(self, entry, num_cf, num_ff):
        ...
        :return:
        """
        (entry, spa_so_rels_feats_tf, obj_seqs_tf, spa_temp_so_rels_feats_tf) = self.generate_spatio_temporal_predicate_embeddings(entry)
        ...
        while num_cf + 1 <= num_tf:
            ...
            temp = self.generate_future_ff_rels_for_context(entry, spa_temp_so_rels_feats_tf, obj_seqs_tf, num_cf, num_tf, num_ff)
            result[count] = temp
            count += 1
            num_cf += 1
    ...
    def forward_single_entry(self, context_fraction, entry):
        ...
        temp = self.generate_future_ff_rels_for_context(entry, spa_temp_so_rels_feats_tf, obj_seqs_tf, num_cf, num_tf,
                                                        num_ff)
