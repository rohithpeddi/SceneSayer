...
train_dataloader = AG_dataloader(data=DATA_PATH, annot_path='../../predcls_gt_annotations', data_split="train")
...
for epoch in range(10, 10 + MAX_EPOCHS):
	...
	torch.save({"state_dict": model.state_dict()}, os.path.join('model_predcls/', "model_{}_lr4.tar".format(epoch)))
	print("*" * 40)
	...
	score = np.mean(evaluator.result_dict["predcls" + "_recall"][20])
	end = time.time()
