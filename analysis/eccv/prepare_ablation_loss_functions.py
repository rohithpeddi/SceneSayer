...
def fetch_method_name_latex(method_name):
	...
	elif method_name == "sde_wo_gen_loss":
...
def generate_ablation_context_recall_results_csvs(context_results_json):
	for mode in modes:
		csv_file_name = f"recall_{mode}.csv"
		csv_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
		                             "ablation_context_results_csvs", csv_file_name)
		os.makedirs(os.path.dirname(csv_file_path), exist_ok=True)
		with open(csv_file_path, "a", newline='') as csv_file:
			writer = csv.writer(csv_file, quoting=csv.QUOTE_NONNUMERIC)
			writer.writerow([
...
def generate_ablation_context_mean_recall_results_csvs(context_results_json):
	for mode in modes:
...
def generate_ablation_future_frame_recall_results_csvs(future_frame_results_json):
	for mode in modes:
		csv_file_name = f"recall_{mode}.csv"
		csv_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
		                             "ablation_future_frame_results_csvs", csv_file_name)
		os.makedirs(os.path.dirname(csv_file_path), exist_ok=True)
		with open(csv_file_path, "a", newline='') as csv_file:
			writer = csv.writer(csv_file, quoting=csv.QUOTE_NONNUMERIC)
			writer.writerow([
...
def generate_ablation_future_frame_mean_recall_results_csvs(future_frame_results_json):
	for mode in modes:
		csv_file_name = f"mean_recall_{mode}.csv"
		csv_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
		                             "ablation_future_frame_results_csvs", csv_file_name)
...
def generate_ablation_context_recalls_latex_tables(context_results_json):
	for mode in modes:
		for train_num_future_frame in train_future_frame_loss_list:
			...
			latex_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
			                               "ablation_context_recalls_latex_tables", latex_file_name)
			...
			for context_fraction in context_fraction_list:
				values_matrix = np.zeros((9, 12), dtype=np.float32)
...
def generate_ablation_wn_context_recalls_latex_tables(context_results_json):
	for mode in modes:
		for train_num_future_frame in train_future_frame_loss_list:
			latex_file_name = f"com_wn_recalls_{mode}_{train_num_future_frame}.txt"
			latex_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
			                               "ablation_wn_context_recalls_latex_tables", latex_file_name)
			os.makedirs(os.path.dirname(latex_file_path), exist_ok=True)
...
def generate_ablation_future_frame_recalls_latex_tables(context_results_json):
	for mode in modes:
		for train_num_future_frame in train_future_frame_loss_list:
			latex_file_name = f"com_recalls_{mode}_{train_num_future_frame}.txt"
			latex_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
			                               "ablation_future_frame_recalls_latex_tables", latex_file_name)
			os.makedirs(os.path.dirname(latex_file_path), exist_ok=True)
...
def generate_ablation_wn_future_frame_recalls_latex_tables(context_results_json):
	for mode in modes:
		for train_num_future_frame in train_future_frame_loss_list:
			latex_file_name = f"com_wn_recalls_{mode}_{train_num_future_frame}.txt"
			latex_file_path = os.path.join(os.path.dirname(__file__), "results_docs",
			                               "ablation_wn_future_frame_recalls_latex_tables", latex_file_name)
			os.makedirs(os.path.dirname(latex_file_path), exist_ok=True)
...
def prepare_ablation_context_results():
	context_results_json = compile_context_results()
	# Generate Context Results CSVs
	generate_ablation_context_recall_results_csvs(context_results_json)
	generate_ablation_context_mean_recall_results_csvs(context_results_json)
	
	# Generate Context LateX Tables
	generate_ablation_context_recalls_latex_tables(context_results_json)
	generate_ablation_wn_context_recalls_latex_tables(context_results_json)


def prepare_ablation_future_frame_results():
	future_frame_results_json = compile_complete_future_frame_results()
	# Generate Future Frame Results CSVs
	generate_ablation_future_frame_recall_results_csvs(future_frame_results_json)
	generate_ablation_future_frame_mean_recall_results_csvs(future_frame_results_json)
	
	# Generate Future Frame Results Latex Tables
	generate_ablation_future_frame_recalls_latex_tables(future_frame_results_json)
	generate_ablation_wn_future_frame_recalls_latex_tables(future_frame_results_json)
...
if __name__ == '__main__':
	...
	methods = ["sttran_ant", "dsgdetr_ant", "sttran_gen_ant", "dsgdetr_gen_ant", "ode", "sde_wo_bb", "sde_wo_recon", "sde_wo_gen_loss", "sde"]
	train_future_frame_loss_list = ["3"]
	...
	prepare_ablation_context_results()
	prepare_ablation_future_frame_results()
