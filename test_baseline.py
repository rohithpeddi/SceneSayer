...
def generate_context_qualitative_results(model, entry, gt_annotation, conf, context_fraction, percentage_evaluators):
	...
	# Draw the predicted graphs for the future frames
	pass
...
def generate_qualitative_results():
	...
	with torch.no_grad():
		for video_id in video_id_list:
			im_data, im_info, gt_boxes, num_boxes, index = ag_test_data.fetch_video_data(video_id_index_map[video_id])
			gt_annotation = ag_test_data.gt_annotations[index]
			for context_fraction in context_fractions:
				entry = object_detector(im_data, im_info, gt_boxes, num_boxes, gt_annotation, im_all=None)
